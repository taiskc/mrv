    ...
    <b-card class="my-2">
      ...
      <div class="d-flex flex-row align-items-center">
        <div class="mr-4 d-flex align-items-center">
          <img class="icon mr-1" src="../assets/icons/map-pin.svg" />{{
            lead.suburb
          }}
        </div>
        <div class="mr-4 d-flex align-items-center">
          <img class="icon mr-1" src="../assets/icons/briefcase.svg" />
          {{ lead.category }}
        </div>
      ...
      <div class="d-flex flex-row align-items-center">
        ...
        <b-button
          class="mr-2"
          variant="warning"
          @click="evaluateLead(true)"
          :disabled="loadingEvaluation"
          >Accept</b-button
        >
        <b-button
          class="mr-2"
          @click="evaluateLead(false)"
          :disabled="loadingEvaluation"
          >Decline</b-button
        >
...
export default {
  ...
  methods: {
    ...
    updateLeads() {
      this.$parent.$emit("updateLeads", this.evaluation, this.leadComplete);
    },
    evaluateLead(evaluation) {
      ...
      fetch("https://localhost:7079/api/lead/evaluate/" + this.lead.id + "/", {
        ...
        .catch((error) => {
          const middleOfMessage = evaluation ? "accepted" : "declined";
          this.modalMessage =
            "There was an error and the lead could not be " +
            middleOfMessage +
            "! Please, try again.";
          this.$refs["generalModal"].show();
